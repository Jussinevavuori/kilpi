...
_ReBAC_ or _Relationship-based access control_ is a model where authorizations to objects are based on relationships between different objects, such as **data ownership**, **parent-child relationships**, **groups**, and **hierarchies**.
...
1. **Pass as object**
   ...
   When the relationship is between the object and the subject, it is best to pass the object as an argument to the policy function directly. See the [Data ownership](#data-ownership) example.

2. **Fetch in policy**

   For data that can not be considered the object of the policy, and is not global enough to be included in the subject, you can fetch in an asynchronous policy. See the [Parent-child relationships example](#parent-child-relationships).
