...
<Step>
### Recommended: Skip subject fetching during build time
...
> Otherwise, Kilpi will most likely attempt to call `headers()` or `cookies()` during build time, which will throw an error. See [this issue](https://github.com/vercel/next.js/discussions/66832) for details.
