...
In addition to returning partial data, you can also return filtered data either manually or by using `Kilpi.filter` to filter a set of objects to only the ones passing a specific policy.

1. Define a policy that accepts an object.
...
2. Call `Kilpi.filter` to get only the objects that pass the policy for the current subject.
