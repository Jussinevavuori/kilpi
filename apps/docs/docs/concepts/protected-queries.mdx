...
description: Learn how to define and use protected queries with Kilpi.$query for co-located authorization logic.
sidebar:
  order: 5
---
...
However, queries are easier to e.g. dedupe or cache if authorization logic is separate from the query function (the query should be kept **pure** — authorization logic is impure).
