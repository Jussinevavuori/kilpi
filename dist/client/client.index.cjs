...
var exports_client_index = {};
__export(exports_client_index, {
  createClient: () => createClient,
  KilpiClient: () => KilpiClient
});
module.exports = __toCommonJS(exports_client_index);
...
// src/lib/create-client-side-cache.ts
function createClientSideCache(cacheTimeSeconds = Infinity) {
  const cache = new Map;
...
// src/lib/create-client-side-value-cache.ts
function createClientSideValueCache(options = {}) {
  const cacheTimeMs = 1000 * (options.cacheTimeSeconds ?? Infinity);
  let updatedAt = null;
  let value = options.intialValue ?? undefined;
  function invalidate() {
    updatedAt = null;
    value = undefined;
  }
...
// src/lib/create-subscribable.ts
...
function createClient(options) {
  ...
  const invalidationSubscribable = createSubscribable();
  function onInvalidate(callback) {
    return invalidationSubscribable.subscribe(callback);
  }
  const subjectCache = createClientSideValueCache();
  const permissionCache = createClientSideCache();
  function invalidate() {
  ...
  const fetchPermissionBatcher = createBatcher(async (rules) => {
    ...
    if (response.status !== 200) {
      throw new KilpiError.FetchPermissionFailed(`Failed to fetch permissions (${response.status} ${response.statusText}): ${await response.text()}`);
    }
    const jsonSchema = z.discriminatedUnion("granted", [
