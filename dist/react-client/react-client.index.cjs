...
// src/react-client/react-client.index.tsx
var exports_react_client_index = {};
__export(exports_react_client_index, {
  createKilpiReactClient: () => createKilpiReactClient
});
module.exports = __toCommonJS(exports_react_client_index);
var import_react = __toESM(require_react());
"use client";
function createKilpiReactClient(KilpiClient, globalOptions = {}) {
  function useInvalidationSignal() {
    const [signal, setSignal] = import_react.useState(0);
    import_react.useEffect(() => KilpiClient.onInvalidate(() => setSignal((_) => _ + 1)), [setSignal]);
    return signal;
  }
  function usePermission(options) {
    ...
    import_react.useEffect(() => {
      ...
      KilpiClient.getPermission(options.to, options.on).then((result) => {
        if (result.granted) {
  ...
  function useSubject() {
    const invalidationSignal = useInvalidationSignal();
    const [subject, setSubject] = import_react.useState({ status: "loading" });
    import_react.useEffect(() => {
      KilpiClient.getSubject().then((subject2) => {
        setSubject({ status: "success", subject: subject2 });
      }).catch((error) => {
        console.error(error);
