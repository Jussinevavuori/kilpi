...
Kilpi is designed to be framework agnostic. Using a pre-made adapter or component library is completely optional.
...
```ts
// Kilpi.ts
// Setup Kilpi and policies
const policies = {
  documents: {
		update: AuthedPolicy.new((user, doc: Document) => {
...
// Query has two explicit methods: protect and unsafe, not callable directly
const authorizedDocument = await getDocument.protect("1");
const unauthorizedDocument = await getDocument.unsafe("2");
```
...
export default async function Page(props) {
	...
	const doc = await getDocument.protect((await props.params).id);
