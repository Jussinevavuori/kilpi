...
The simplest way to check for authorizations is to use `Kilpi.isAuthorized`. It returns a `Promise<boolean>` indicating whether the user is authorized or not. Pass it the policy key and an object, if the policy requires one.
...
`Kilpi.authorize` also takes in the policy key and optionally an object if the policy requires one.
