...
In Kilpi, queries are primarily protected with [protected queries](/docs/concepts/protected-query) created using `Kilpi.query`. [Read more](/docs/concepts/protected-query) to understand why Kilpi uses protected queries.
...
```
> If you are using [throw -based APIs](/docs/concepts/protection-basics), query protectors should throw when unauthorized as above. Otherwise, you can use e.g. `Kilpi.isAuthorized` or `Kilpi.getAuthorization` and return `null` or similar to signal unauthorized.
