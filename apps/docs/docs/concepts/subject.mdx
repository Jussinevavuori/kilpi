...
<Aside>
  **Subject ≈ user**, however the term **subject** is more commonly used in
...
The current user is provided using the **`getSubject` adapter** to allow getting the subject from any system or authentication provider. Because authentication always comes before authorization, Kilpi reduces boilerplate and calls `getSubject` for you.
...
This can be done in multiple ways, and some plugins (such as [`ReactServerPlugin`](/docs/plugins/react-server-plugin)) even automatically caches the subject for each request.
