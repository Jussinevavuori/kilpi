...
:::note
The subject is automatically cached inside the scope, when available. To disable this behavior, you can pass `settings.disableSubjectCaching: true` to `createKilpi`. (We heavily recommend caching the subject for performance reasons, either yourself or by using the default caching behavior).
