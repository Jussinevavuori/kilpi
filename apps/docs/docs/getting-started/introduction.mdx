...
Kilpi is designed around a centralized authorization layer consisting of a set of policies, uniquely mapping each action, such as `documents:update`, to a functional policy. Reducing duplicated code makes your authorization logic easier to maintain and understand.
