...
// packages/react/src/client/index.tsx
var exports_client = {};
__export(exports_client, {
  createKilpiReactClientComponents: () => createKilpiReactClientComponents
});
module.exports = __toCommonJS(exports_client);
var import_react = __toESM(require_react());
function createKilpiReactClientComponents(KilpiClient, options) {
  function useInvalidationSignal(KilpiClient2) {
    const [signal, setSignal] = import_react.useState(0);
    import_react.useEffect(() => KilpiClient2.onInvalidate(() => setSignal((_) => _ + 1)), [setSignal]);
    return signal;
  }
  function useSubject({
    KilpiClient: KilpiClient2
  }) {
    const [subject, setSubject] = import_react.useState({
      status: "loading"
    });
    import_react.useEffect(() => {
      KilpiClient2.getSubject().then((subject2) => setSubject({ status: "success", subject: subject2 })).catch((error) => {
        console.error(error);
        setSubject({ status: "error" });
      });
    }, [KilpiClient2, setSubject, useInvalidationSignal(KilpiClient2)]);
    return subject;
  }
  function usePermission(key, resource) {
    const [permission, setPermission] = import_react.useState({
      status: "loading"
    });
    import_react.useEffect(() => {
      KilpiClient.getPermission(key, resource).then((result) => {
        if (result.granted) {
