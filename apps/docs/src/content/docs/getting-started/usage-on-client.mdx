...
You can also use Kilpi on the client-side, as long as you have a server-side instance of Kilpi you can connect to using the [endpoint plugin](https://kilpi.dev/docs/plugins/endpoint).
...
3.  ### Provide an authorization endpoint
    ...
    Use the [EndpointPlugin](/plugins/endpoint) provided by `@kilpi/core` to create an authorization endpoint. It exposes the `Kilpi.createPostEndpoint()` function, which constructs a web-standard request-response function to use as your endpoint.
...
6.  ### Integrate into your frontend framework of choice
    ...
    <Tabs syncKey="frontend">
      <TabItem label="React">
        ...
        View [the documentation](/plugins/react-client) for the React Client plugin to start using it.
      </TabItem>
    </Tabs>
