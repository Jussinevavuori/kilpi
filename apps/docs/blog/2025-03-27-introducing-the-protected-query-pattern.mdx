...
Creating a protected query is simple with [Kilpi.query](https://kilpi.vercel.app/docs/concepts/protected-queries), but you can also implement a similar pattern on your own.

```ts
const getDocument = Kilpi.query(
  cache(async (id: string) => {
    // Pure query function can be cached!
