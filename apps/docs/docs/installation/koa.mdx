...
However, since Koa's errors are thrown with `ctx.throw`, the global `onUnauthorizedAssert` handler requires access to `ctx`. The best way to achieve this is using `AsyncLocalStorage` as shown below.
