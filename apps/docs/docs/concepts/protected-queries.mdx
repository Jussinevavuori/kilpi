...
Co-locating queries and authorization logic maintaining and developing applications easier. However, queries are easier to e.g. dedupe or cache if authorization logic is separate from the query function (the query should be kept **pure** — authorization logic is impure).
