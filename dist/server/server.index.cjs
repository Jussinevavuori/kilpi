...
var exports_server_index = {};
__export(exports_server_index, {
  createRuleset: () => createRuleset,
  createClient: () => createClient,
  KilpiServer: () => KilpiServer
});
module.exports = __toCommonJS(exports_server_index);
...
// src/server/setup-create-query.ts
function setupCreateQuery(options = {}) {
  return function createQuery(originalQuery, protector) {
    const query = (options.plugins ?? []).reduce((q, plugin) => plugin.wrapProtectedQuery ? plugin.wrapProtectedQuery(q) : q, originalQuery);
...
// src/server/setup-endpoint.ts
function setupEndpoint({
  getSubject,
  ruleset
}) {
  return function createPostEndpoint({ secret }) {
        ...
        switch (body.action) {
          ...
          case "fetchPermission": {
            const permissions = Promise.all(body.rules.map(async ({ key, resource }) => {
              const rule = getRuleByKey(ruleset, key);
              const subject = await getSubject();
...
// src/server/setup-protectors.ts
function setupProtectors(getSubject, ruleset, options) {
  const protectCallStackSizeThreshold = 50;
  let protectCallStackSize = 0;
  async function protect(ruleKey, resource) {
    protectCallStackSize++;
    if (protectCallStackSize > protectCallStackSizeThreshold) {
      throw new Error("\uD83D\uDEA8 Kilpi.protect() called too many times recursively. This is" + "usually caused by a rule fetching a resource using a protected " + "query, causing a recursive loop. Ensure no rule uses protected " + "queries or otherwise calls protect().");
    }
    const subject = await getSubject();
  ...
  async function getPermission(ruleKey, resource) {
    const subject = await getSubject();
    const rule = getRuleByKey(ruleset, ruleKey);
    const permission = await rule.getPermission(subject, resource);
    return permission;
  }
  async function hasPermission(ruleKey, resource) {
    const subject = await getSubject();
    const rule = getRuleByKey(ruleset, ruleKey);
    const permission = await rule.getPermission(subject, resource);
...
// src/server/create-ruleset.ts
function createRuleset() {
  return function craeteRulesetImpl(rules) {
    return rules(initializeRules());
  };
}
...
function createClient({
  ...
  const ruleset = typeof rules === "function" ? rules(initializeRules()) : rules;
  ...
  function onProtect(onProtectFn) {
    if (!onProtectStorage)
      return;
  ...
  return {
    ...
    protect,
    onProtect,
    ...Object.values(plugins ?? []).reduce((interfaces, plugin) => {
