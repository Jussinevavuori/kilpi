...
<FileTree>
  - src - kilpi - policies.ts Define your policies here - subject.ts Define your subject here -
...
<FileTree>
  - src - kilpi - policies - organizations.policies.ts - documents.policies.ts - media.policies.ts -
  subject.ts - kilpi.ts
</FileTree>
  ...
  <Step>
    ...
    Create `policies.ts` where you define all your policies as functions that take in the `user` and optionally an object, and return `grant(user)` or `deny()`.
