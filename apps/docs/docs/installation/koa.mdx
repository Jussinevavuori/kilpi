...
The `.assert()` method throws an error when authorization is denied. Koa surfaces errors via `ctx.throw`, so the global `onUnauthorizedAssert` handler requires access to `ctx`. One way to achieve this is using `AsyncLocalStorage` as shown below.
