...
Throwing on unauthorized requires either a [scope](/docs/concepts/scopes) with an error handler set via `Kilpi.onUnauthorized` or a global default error handler passed to `createKilpi` as `defaults.onUnauthorized`.
